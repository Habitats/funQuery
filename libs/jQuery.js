...
(function(window, undefined) {
	...
	(function(window, undefined) {
		...
		var cachedruns, assertGetIdNotName, Expr, getText, isXML, contains, compile, sortOrder, hasDuplicate, outermostContext, baseHasDuplicate = true, strundefined = "undefined", expando = ("sizcache" + Math
				.random()).replace(".", ""), Token = String, document = window.document, docElem = document.documentElement, dirruns = 0, done = 0, pop = [].pop, push = [].push, slice = [].slice,
		...
		rtrim = new RegExp("^" + whitespace + "+|((?:^|[^\\\\])(?:\\\\.)*)"
				...
				+ whitespace + "*([\\x20\\t\\r\\n\\f>+~])" + whitespace + "*"), rpseudo = new RegExp(
				pseudos),
		...
		function setMatcher(preFilter, selector, matcher, postFilter,
			...
			return markFunction(function(seed, results, context, xml) {
				...
				matcherIn = preFilter && (seed || !selector) ? condense(elems,
						preMap, preFilter, context, xml) : elems, matcherOut = matcher ?
	...
	rdisplayswap = /^(none|table(?!-c[ea]).+)/, rmargin = /^margin/, rnumsplit = new RegExp(
		...
		letterSpacing : 0,
		fontWeight : 400
	}, cssExpand = [ "Top", "Right", "Bottom", "Left" ], cssPrefixes = [
			"Webkit", "O", "Moz", "ms" ], eventsToggle = jQuery.fn.toggle;
